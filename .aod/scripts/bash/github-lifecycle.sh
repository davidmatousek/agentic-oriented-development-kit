...
aod_gh_add_to_board() {
    ...
    local column_name="${AOD_STAGE_TO_COLUMN[$stage]:-}"
    ...
    local item_id
    item_id=$(echo "$add_result" | jq -r '.id' 2>/dev/null)
...
aod_gh_move_on_board() {
    ...
    local column_name="${AOD_STAGE_TO_COLUMN[$new_stage]:-}"
...
aod_gh_update_stage() {
    ...
    issue_url=$(gh issue view "$issue_number" --json url --jq '.url' 2>/dev/null) || true
    if [[ -n "$issue_url" ]]; then
        aod_gh_move_on_board "$issue_url" "$new_stage" >/dev/null 2>&1 || true
    fi
