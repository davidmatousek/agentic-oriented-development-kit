...
Vercel for Next.js hosting (production + automatic preview deployments per PR), Supabase managed PostgreSQL + Auth + Storage + Realtime (separate projects per environment), GitHub Actions for CI pipeline, Prisma for database schema migrations, pnpm as package manager, Biome for linting and formatting, environment variables managed through Vercel Dashboard.
...
- ALWAYS run the full CI gate in GitHub Actions before merge: `pnpm biome check .` → `pnpm tsc --noEmit` → `pnpm test` → `next build`
- ALWAYS pin exact dependency versions in `package.json` -- no `^` or `~` prefixes
- ALWAYS commit `pnpm-lock.yaml` to git -- never delete or gitignore it
...
- NEVER use npm or yarn -- use pnpm exclusively for install, scripts, and lockfile consistency
...
- NEVER delete or regenerate `pnpm-lock.yaml` to fix dependency issues -- resolve conflicts properly

## Guardrails

- Required CI checks before merge: Biome lint (`pnpm biome check .`), TypeScript (`pnpm tsc --noEmit`), Vitest (`pnpm test`), Next.js build (`next build`)
